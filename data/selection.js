...
ShanbayChromeExtension._resultDivSelector = "#" + ShanbayChromeExtension._resultDivId;
...
ShanbayChromeExtension._setlisten=false;
//引擎类
ShanbayChromeExtension._Engine = function(index, meta) {
	...
	if ( typeof ShanbayChromeExtension._Engine._initialized == "undefined") {
		...
		//error handler
		ShanbayChromeExtension._Engine.prototype._onError = function(xhr, type, cause) {
			return ( typeof this.error === "undefined" ? this.$defaultError(xhr, type, cause) : this.error(xhr, type, cause));
		}
	}

	//default error message
	ShanbayChromeExtension._Engine.prototype.$defaultError = function(xhr, type, cause) {
		return [this.name, type + ": " + xhr.status + " " + cause];
	}

	ShanbayChromeExtension._Engine.prototype._validateText = function(text, options) {
		var option = options[this.id];
		this.$option = option;

		if ((option && option.enabled === false) || this.enabled === false || this.filter(text) === false) {
	...
	ShanbayChromeExtension._Engine.prototype._queryAndShow = function(text) {
		...
		success: this.$bind("parser"),
		error: this.$bind("_onError")
		})*/

		// firefox query
       // console.log(url);
       var obj={id:this._engineDivSelector,url:url};
		self.port.emit('queryshanbay', obj);
        //console.log(this._engineDivSelector);
       
        if(portListens[obj.id+'success']==null){
        portListens[obj.id+'success']=self.port.on(obj.id+'success',this.$bind("parser") );
        portListens[obj.id+'fail']=self.port.on(obj.id+'fail',this.$bind("_onError") );
        }
		//   $.get(url,null,this.$bind("parser"),'json');
	...
	ShanbayChromeExtension._Engine.prototype._showResult = function(htmls) {
		...
		for (var i in selectors) {
			if (htmls[i] != null) {
				...
				$(selectors[i]).append(htmls[i]);
			}
		}
		// console.log($(ShanbayChromeExtension._resultDivSelector).html());
		//  $(this._contentDivSelector).slideDown("slow");
	/*	var htmlobj = {
			id : this._contentDivSelector,
			content : $(ShanbayChromeExtension._resultDivSelector).html()
		};*/

		//self.port.emit('showresult',$(ShanbayChromeExtension._resultDivSelector).html());
		//self.port.emit('showresult', htmlobj);
		$(this._contentDivSelector).slideDown("slow");
       // console.log($(this._contentDivSelector).html());
	}
	...
	//生成一个在新窗口打开的链接，返回字符串。
	ShanbayChromeExtension._Engine.prototype.$generateLink = function(linkText, href) {
	...
	ShanbayChromeExtension._Engine.prototype.$generateClickableLink = function(linkText, handlerName) {
...
ShanbayChromeExtension._initResultDiv = function() {
	...
	for (i in this._engines) {
        ...
        engineDiv.style.background='white';
        engineDiv.style.fontSize='14px';
...
ShanbayChromeExtension.onSelect = function(event) {
	...
	$(this._resultDivSelector).css("left", x + "px").css("top", event.pageY + 10 + "px");
...
ShanbayChromeExtension.queryAndShow = function(text, options) {
	...
	if (flag === true) {
		$(this._resultDivSelector).fadeIn('normal');
	}
}

$("body").ready(function() {
	ShanbayChromeExtension.initialize();
/*	self.port.on('select', function(event) {
