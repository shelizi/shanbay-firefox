    ...
    .concat([
        {
          ...
          urlAdd : "http://www.shanbay.com/api/learning/add/{{text}}",
        ...
        //  var result=$.parseJSON(result);
          ...
          add : function() { //添加单词
            var url = this.urlAdd.replace("{{text}}", this.$text);
            var thisEngine = this;
            //$.getJSON(url, this.$bind("addCallback"));
            self.port.emit('addword',url);
            if(addlistener==false)
            {
                addlistener=true;
                self.port.on('addWordResponse',this.$bind("addCallback"));
          ...
          getTitle : function(result, autoplay) {
            var caption = this.$$generateTitle(result.voc.content);
            if (result.voc.pron)
              caption.html(caption.text() + "[" + result.voc.pron + "]");

            if (result.voc.audio) {
              this.audioId = this.$generateId("audio");
              props = {
                id : this.audioId,
                src : result.voc.audio,
              };
              if (autoplay != false) {
        ...
        },
        {
          ...
          name : "谷歌翻译",
          enabled : true,
          url : "http://translate.google.com/translate_a/t?client=t&text={{text}}&hl=en&sl=auto&tl=zh-TW&multires=1&otf=2&ssel=0&tsel=0&uptl=zh-TW&sc=1",
        ...
        },
        {
          ...
          name : "维基百科",
          enabled : true,
          url : "http://zh.wikipedia.org/zh-tw/{{text}}",
          type : "html",
          filter : function(text) {
            if (text.length > 30) {
              return false;
